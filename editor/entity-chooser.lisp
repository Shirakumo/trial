...
(define-widget entity-chooser (QDialog)
  ())

(define-initializer (entity-chooser setup)
  (setf (q+:window-title entity-chooser) "Entity Chooser")
  (q+:resize entity-chooser 300 500)
  (refresh-entities entity-chooser))

(define-subwidget (entity-chooser filter)
    (q+:make-qlineedit "subject"))

(define-subwidget (entity-chooser clear-filter)
    (q+:make-qpushbutton)
...
(define-subwidget (entity-chooser result-list)
    (make-instance 'class-listing))

(define-subwidget (entity-chooser scroller)
    (q+:make-qscrollarea)
...
(define-subwidget (entity-chooser refresh)
    (q+:make-qpushbutton)
...
(define-subwidget (entity-chooser inspect)
    (q+:make-qpushbutton)
...
(define-subwidget (entity-chooser list-instances)
    (q+:make-qpushbutton)
...
(define-subwidget (entity-chooser layout)
    (q+:make-qgridlayout entity-chooser)
...
(define-slot (entity-chooser refresh refresh-entities) ()
...
(define-slot (entity-chooser list-instances) ()
  ...
  (let ((selected (qui:active-item result-list)))
    (cond ((not selected))
          ...
          (T
           (q+:qmessagebox-warning entity-chooser "Error" "Can't show instances of a non-subject subclass.")))))
