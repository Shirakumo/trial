...
(defun inspect (object)
  (typecase object
    ...
    ((or cons array hash-table package symbol)
     #+swank (swank:inspect-in-emacs object))))

(define-widget object-inspector (QDialog)
...
(define-subwidget (object-inspector layout)
  ...
  (q+:add-widget layout finalize 4 1 1 1)
...
(define-slot (slot-listing-widget set-value) ()
  ...
  (let* ((object (object (qui:container slot-listing-widget)))
         (slot (qui:widget-item slot-listing-widget))
         (new (q+:qinputdialog-get-text
               slot-listing-widget (format NIL "Set a new value for ~s" slot)
        ...
        (let* ((expr (handler-case (read-from-string new)
                       (error (err)
                         (q+:qmessagebox-critical
                          slot-listing-widget "Error during reading of new value"
                          (princ-to-string err))
                         (return))))
               (value (handler-case (eval expr)
                        (error (err)
                          (q+:qmessagebox-critical
                           slot-listing-widget "Error during evaluation of new value"
                           (princ-to-string err))
                          (return)))))
