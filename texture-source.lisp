    ...
    (mem:with-memory-region (region data)
      (destructuring-bind (src-x src-y src-z src-w src-h src-d) (texture-source-src source)
        (destructuring-bind (dst-x dst-y dst-z dst-w dst-h dst-d) (texture-source-dst source)
          ...
          (ecase target
            (:texture-1d
             (gl:pixel-store :unpack-skip-pixels src-x)
             (%gl:tex-sub-image-1d target level dst-x dst-w format type (memory-region-pointer region)))
            ((:texture-2d :texture-1d-array)
             (gl:pixel-store :unpack-skip-pixels src-x)
             (gl:pixel-store :unpack-skip-rows src-y)
             (gl:pixel-store :unpack-row-length src-w)
             (%gl:tex-sub-image-2d target level dst-x dst-y dst-w dst-h format type (memory-region-pointer region)))
            ((:texture-cube-map)
             (gl:pixel-store :unpack-skip-pixels src-x)
             (gl:pixel-store :unpack-skip-rows src-y)
             (gl:pixel-store :unpack-row-length src-w)
             (%gl:tex-sub-image-2d (target source) level dst-x dst-y dst-w dst-h format type (memory-region-pointer region)))
