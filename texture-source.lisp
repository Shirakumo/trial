...
(defun texture-sources->width (sources)
  (let ((max-x 0))
    (dolist (source sources)
      (destructuring-bind (x y z w h d) (texture-source-src source)
        (declare (ignore y z h d))
        (setf max-x (max max-x (+ (or x 0) (or w 0))))))
    (unless (= 0 max-x) max-x)))

(defun texture-sources->height (sources)
  (let ((max-y 0))
    (dolist (source sources)
      (destructuring-bind (x y z w h d) (texture-source-src source)
        (declare (ignore x z w d))
        (setf max-y (max max-y (+ (or y 0) (or h 0))))))
    (unless (= 0 max-y) max-y)))

(defun texture-sources->depth (sources)
  (let ((max-z 0))
    (dolist (source sources)
      (destructuring-bind (x y z w h d) (texture-source-src source)
...
(declaim (inline texture-sources->texture-size))
