...
(defun frictionful-impulse (contact &optional (impulse (vec3 0 0 0)))
  ...
  (flet ((delta-vel (loc inverse-inertia-tensor)
    ...
    (let* ((inverse-mass (+ (the single-float (inverse-mass (contact-a contact)))
           ...
           (delta-vel-world (nm+ (delta-vel (contact-a-relative contact)
      ...
      (let* ((impulse-matrix (nminv delta-velocity))
             (impulse-contact (n*m impulse-matrix impulse))
             (planar-impulse (sqrt (the (single-float 0.0)
                                        (+ (* (vy impulse-contact) (vy impulse-contact))
                                           (* (vz impulse-contact) (vz impulse-contact))))))
             (friction (contact-dynamic-friction contact)))
        (when (< (* (vx impulse-contact) (contact-static-friction contact)) planar-impulse)
          (setf (vy impulse-contact) (/ (vy impulse-contact) planar-impulse))
          (setf (vz impulse-contact) (/ (vz impulse-contact) planar-impulse))
          (setf (vx impulse) (/* (contact-desired-delta contact)
                                 (+ (miref delta-velocity 0)
                                    (* (miref delta-velocity 1) friction (vy impulse-contact))
                                    (* (miref delta-velocity 2) friction (vz impulse-contact)))))
          (setf (vy impulse) (* (vy impulse-contact) friction (vx impulse-contact)))
          (setf (vz impulse) (* (vz impulse-contact) friction (vx impulse-contact))))
        impulse))))
