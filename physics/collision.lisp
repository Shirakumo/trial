    ...
    (when data
      (let ((axis (getf data :normal))
        ...
        (when (< 0.0 (v. (getf data :normal)
                         (v- (location (getf data :parent))
                             loc-other)))
          (nv* axis -1.0)
          (setf (getf data :normal) axis))
        (for:for ((point in (mass-points (getf data :parent)))
                  (loc = (location point))
...
(defmethod project-to-axis ((entity collidable-verlet-entity) axis)
  (let* ((dotp (v. (v* axis (offset-location entity))))
         (min dotp)
         (max dotp))
    (for:for ((point in (mass-points entity))
              (loc = (location point))
