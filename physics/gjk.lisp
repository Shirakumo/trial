...
(declaim (inline p<-))
(defun p<- (target a)
  (declare (type point target a))
  (v<- target a)
  (v<- (point-a target) (point-a a))
  (v<- (point-b target) (point-b a)))
...
(defun epa (s0 s1 s2 s3 a b hit)
  ...
  (let ((faces (make-array (* 4 EPA-MAX-FACES)))
    ...
    (dotimes (i EPA-MAX-FACES)
      (setf (aref faces (+ (* i 4) 0)) (point))
      (setf (aref faces (+ (* i 4) 1)) (point))
      (setf (aref faces (+ (* i 4) 2)) (point))
      (setf (aref faces (+ (* i 4) 3)) (vec3)))
    ...
    (macrolet ((v (f v)
      ...
      (p<- (v 0 1) s1)
      (p<- (v 0 2) s2)
      (nvunit* (!vc (v 0 3) (v- s1 s0) (v- s2 s0)))
      (p<- (v 1 0) s0)
      (p<- (v 1 1) s2)
      (p<- (v 1 2) s3)
      (nvunit* (!vc (v 1 3) (v- s2 s0) (v- s3 s0)))
      (p<- (v 2 0) s0)
      (p<- (v 2 1) s3)
      (p<- (v 2 2) s1)
      (nvunit* (!vc (v 2 3) (v- s3 s0) (v- s1 s0)))
      (p<- (v 3 0) s1)
      (p<- (v 3 1) s3)
      (p<- (v 3 2) s2)
      (nvunit* (!vc (v 3 3) (v- s3 s1) (v- s3 s1)))
      ...
      (let ((p (vec3)) (local-a (vec3)) (local-b (vec3)))
        ...
        (barycentric (v closest-face 0) (v closest-face 1) (v closest-face 2)
                     ...
                     p)
        (nv+* local-a (point-a (v closest-face 0)) (vx p))
        (nv+* local-a (point-a (v closest-face 1)) (vy p))
        (nv+* local-a (point-a (v closest-face 2)) (vz p))
        (nv+* local-b (point-b (v closest-face 0)) (vx p))
        (nv+* local-b (point-b (v closest-face 1)) (vy p))
        (nv+* local-b (point-b (v closest-face 2)) (vz p))
        (v<- (trial:hit-location hit) local-b)
        (v<- (trial:hit-normal hit) local-b)
        (nv- (trial:hit-normal hit) local-a)
