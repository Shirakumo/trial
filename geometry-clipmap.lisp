...
(defmethod paint ((clipmap clipmap) (pass shader-pass))
  ...
  (gl:bind-texture :texture-2D (resource (texture clipmap)))
...
(defun draw-ring (clipmap m s shader level scale off f)
  ...
  (flet ((d (x z)
           ...
           (setf (uniform shader "FineBlockOrig") (vec4 scale scale (+ 0.5 (* x scale) off) (+ 0.5 (* z scale) off)))
...
void main(){
   ...
   uv = position.xz * FineBlockOrig.xy + FineBlockOrig.zw;

   vec2 alpha = clamp(abs(worldPos/ScaleFactor.xy)*9-4, 0, 1);
   a = max(alpha.x, alpha.y);

   float zf = textureLod(texture_image, uv, level).r;
   float zd = textureLod(texture_image, uv, max(level-1, 0)).r;
   z = ((1-a) * zf + a * zd) * 0.5;
...
//    color = s * vec4(max(1.0, z+0.2), 0, 0, 1);
    color = texture2D(texture_image, uv);
    color.r += a;
}")
