...
(defmethod setup-ui ((scene spectrogram-scene) panel)
  (let* ((layout (make-instance 'alloy:flow-layout))
    ...
    (macrolet ((row (label repr input &rest args)
      ...
      (row "Volume" (setting* 1.0 :spectrogram :magnitude-scale) 'alloy:ranged-wheel :range '(0.0 . 1.0) :grid 0.01 :step 0.1)
      (row "Threshold" (setting* 0.0 :spectrogram :magnitude-offset) 'alloy:ranged-wheel :range '(0.0 . 1.0) :grid 0.01 :step 0.1)
      (row "Min Freq" (setting* 0.0 :spectrogram :frequency-min) 'alloy:ranged-wheel :range '(0.0 . 22049.0) :step 100.0)
      (row "Max Freq" (setting* 22049.0 :spectrogram :frequency-max) 'alloy:ranged-wheel :range '(0.0 . 22049.0) :step 100.0)
      (row "Time Scale" (setting* 1.0 :spectrogram :time-scale) 'alloy:ranged-wheel :range '(1.0 . 10.0)))
...
(defmethod render :before ((visualizer spectrogram-visualizer) (program shader-program))
  (let ((i (i visualizer))
    ...
    (setf (uniform program "magnitude_scale") (setting* 1.0 :spectrogram :magnitude-scale))
    (setf (uniform program "magnitude_offset") (setting* 0.0 :spectrogram :magnitude-offset))
    (setf (uniform program "frequency_min") (setting* 0.0 :spectrogram :frequency-min))
    (setf (uniform program "frequency_max") (setting* 22049.0 :spectrogram :frequency-max))
    (setf (uniform program "time_scale") (setting* 1.0 :spectrogram :time-scale))
