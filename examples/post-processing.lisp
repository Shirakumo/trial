  ...
  (let* ((layout (make-instance 'alloy:grid-layout :col-sizes '(120 200 T) :row-sizes '(30)))
    ...
    (macrolet ((row (label repr input &rest args)
      ...
      (labels ((repopulate ()
                 ...
                 (alloy:clear layout)
                 (alloy:clear focus)
                 (let ((combo (row "Effect" class 'alloy:combo-set :value-set '(copy-pass negative-pass box-blur-pass sobel-pass gaussian-blur-pass radial-blur-pass swirl-pass fxaa-pass high-pass-filter low-pass-filter chromatic-aberration-filter))))
