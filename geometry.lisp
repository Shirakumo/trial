  ...
  (let ((vertices (gensym "VERTICES"))
    ...
    `(let ((,vertices (make-array 0 :element-type 'single-float :adjustable T))
       ...
       (labels ((,constructor (&rest bits)
                  (let* ((c bits)
                         ,@(when deduplicate `((e (gethash c ,face-table)))))
                    (cond ,@(when deduplicate `((e (vector-push-extend e ,faces))))
                          (T
                           ,@(when deduplicate `((vector-push-extend ,i ,faces)))
                           (let ((j (* ,stride ,i)))
                ...
                (,finalizer ()
                  (values (make-array (length ,vertices) :element-type 'single-float :initial-contents ,vertices)
                          ,@(when deduplicate `((make-array (length ,faces) :element-type '(unsigned-byte 16) :initial-contents ,faces)))))
