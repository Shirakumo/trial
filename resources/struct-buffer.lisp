  ...
  (mem:with-memory-region (region (storage (buffer-data buffer)))
    (let ((vbo (gl:gen-buffer)))
      (with-cleanup-on-failure (progn (gl:delete-buffers (list vbo))
        ...
        (let ((usage (ecase (data-usage buffer)
          ...
          (gl-extension-case
            (:GL-ARB-BUFFER-STORAGE
             (%gl:buffer-storage (buffer-type buffer) (memory-region-size region) (memory-region-pointer region)
                                 (ecase (data-usage buffer)
                                   ...
                                   ((:dynamic-copy :stream-copy :static-copy)
                                    (list* :dynamic-storage :map-read :map-write (map-bits buffer))))))
            (T
             (%gl:buffer-data (buffer-type buffer) (memory-region-size region) (memory-region-pointer region)
                              (data-usage buffer)))))))))
