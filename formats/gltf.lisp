...
(defun load-vertex-attribute (mesh attribute accessor skin)
  (let ((data (vertex-data mesh))
    ...
    (cond ((null offset)
           (v:warn :trial.gltf "Ignoring attribute ~s" attribute))
          ((eq 'joints (vertex-attribute-category attribute))
          ...
          (T
           (ecase (vertex-attribute-size attribute)
             (1
              (loop for i from 0 below (length accessor)
                    for el = (elt accessor i)
                    do (setf (aref data (+ (* i stride) offset)) el)))
             (2
              (loop for i from 0 below (length accessor)
                    for el = (elt accessor i)
             ...
             (4
              (loop for i from 0 below (length accessor)
                    for el = (elt accessor i)
                    do (setf (aref data (+ (* i stride) offset 0)) (qx 0))
                       (setf (aref data (+ (* i stride) offset 1)) (qy 1))
                       (setf (aref data (+ (* i stride) offset 2)) (qz 2))
                       (setf (aref data (+ (* i stride) offset 3)) (qw 3)))))))))
          ...
          do (when (gltf:mesh node)
               (loop for primitive across (gltf:primitives (gltf:mesh node))
                     ...
                     for mesh = (if (gltf:skin node)
                                    (make-instance 'skinned-mesh :name name :vertex-attributes attributes)
                                    (make-instance 'static-mesh :name name :vertex-attributes attributes))
                     do (vector-push-extend mesh meshes)
                        ...
                        (print (glsl-toolkit:serialize (gl-source mesh)))
