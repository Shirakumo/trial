...
;; FIXME: How do we determine specs that are only matchable through down/upgrading a spec to
;;        a more specific one? Eg a spec that says :r internal, and another that says :r32f
(defun texspec-matching-p (a b)
  )
...
(defmethod pack-pipeline ((pipeline pipeline) target)
  ...
  (let* ((passes (flow:topological-sort (nodes pipeline)))
         ...
         (texpsecs (mapcar #'texspec (mapcan #'flow:ports passes))))
    (clear-pipeline pipeline)
    (dolist (texpec (remove-duplicates texspecs :test #'texspec-matching-p))
      (allocate-textures passes textures (lambda (port) (texspec-matching-p (texpsec port texspec)))))
