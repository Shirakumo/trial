...
void morph_vertex(inout vec3 position, inout vec3 normal, inout vec2 uv){
  ...
  for(int i=0; i<min(morph_data.count, SIMULTANEOUS_MORPHS); ++i){
    ...
    position += weight * texelFetch(morph_targets, ivec2(vid+0, index), 0).xyz;
    normal += weight * texelFetch(morph_targets, ivec2(vid+1, index), 0).xyz;
    uv += weight * texelFetch(morph_targets, ivec2(vid+2, index), 0).xy;
  }
}
