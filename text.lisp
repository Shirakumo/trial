    ...
    (macrolet ((vertex (&rest vals)
      ...
      (flet ((print-letter (char)
               (let* ((c (clamp 0 (- (char-code char) (char-code #\Space)) 95))
                      (u0 (* gw (+ 0 c)))
                      (u1 (* gw (+ 1 c))))
                 (vertex (* scale (+ x 0.0)) (* scale (+ y 0.0)) z u0 0.0)
                 (vertex (* scale (+ x  gw)) (* scale (+ y 0.0)) z u1 0.0)
                 (vertex (* scale (+ x  gw)) (* scale (+ y  gh)) z u1  gh)
                 (vertex (* scale (+ x  gw)) (* scale (+ y  gh)) z u1  gh)
                 (vertex (* scale (+ x 0.0)) (* scale (+ y  gh)) z u0  gh)
                 (vertex (* scale (+ x 0.0)) (* scale (+ y 0.0)) z u0 0.0)
                 (incf x gw))))
