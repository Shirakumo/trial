...
(define-handler (freeroam-camera tick) (ev)
  (let* ((loc (location freeroam-camera))
    ...
    (cond ((retained 'key :space)
           (incf (vx loc) (* speed (vx rot) (sin (vy rot))))
           (decf (vz loc) (* speed (vx rot) (cos (vy rot))))
           (incf (vy loc) (* speed (cos (vx rot)))))
          ((retained 'key :c)
           (decf (vx loc) (* speed (vx rot) (sin (vy rot))))
           (incf (vz loc) (* speed (vx rot) (cos (vy rot))))
           (decf (vy loc) (* speed (cos (vx rot))))))))
