...
(defun load-mesh-attribute (mesh attribute accessor skin)
  (let ((data (vertex-data mesh)))
    (when (< (length data) (length accessor))
      (setf data (adjust-array data (* (length accessor) 3 3 2 4 4) :element-type 'single-float))
    ...
    (ecase attribute
      (:position
       (loop for i from 0 below (length accessor)
             ...
             do (setf (aref data (+ (* i 3 3 2 4 4) 0)) (vx el))
                (setf (aref data (+ (* i 3 3 2 4 4) 1)) (vy el))
                (setf (aref data (+ (* i 3 3 2 4 4) 2)) (vz el))))
      (:texcoord_0
       (loop for i from 0 below (length accessor)
             for el = (elt accessor i)
             do (setf (aref data (+ (* i 3 3 2 4 4) 3)) (vx el))
                (setf (aref data (+ (* i 3 3 2 4 4) 4)) (vy el))))
      (:joints_0
       (flet ((map-joint (joint)
                (float (max 0 (gltf:idx (svref (gltf:joints skin) joint))) 0f0)))
         (loop for i from 0 below (length accessor)
               for el = (elt accessor i)
               do (setf (aref data (+ (* i 3 3 2 4 4) 5)) (map-joint (aref el 0)))
                  (setf (aref data (+ (* i 3 3 2 4 4) 6)) (map-joint (aref el 1)))
                  (setf (aref data (+ (* i 3 3 2 4 4) 7)) (map-joint (aref el 2)))
                  (setf (aref data (+ (* i 3 3 2 4 4) 8)) (map-joint (aref el 3))))))
      (:weights_0
       (loop for i from 0 below (length accessor)
             for el = (elt accessor i)
             do (setf (aref data (+ (* i 3 3 2 4 4) 9)) (qx el))
                (setf (aref data (+ (* i 3 3 2 4 4) 10)) (qy el))
                (setf (aref data (+ (* i 3 3 2 4 4) 11)) (qz el))
                (setf (aref data (+ (* i 3 3 2 4 4) 12)) (qw el)))))))
...
(defun load-meshes (gltf)
  (let ((meshes (make-array 0 :adjustable T :fill-pointer T)))
    (loop for node across (gltf:nodes gltf)
          ...
          do (when (and skin (gltf:mesh node))
                     ...
                     do (vector-push-extend mesh meshes)
                        ...
                        (when (gltf:indices primitive)
                          (let* ((accessor (gltf:indices primitive))
                                 (indexes (make-array (length accessor) :element-type '(unsigned-byte 16))))
