...
(defmethod reorder ((skeleton skeleton) map)
  (let ((rest (rest-pose skeleton))
    ...
    (clrhash backward)
    (map-into hierarchy (lambda () (make-array 0 :adjustable T :fill-pointer T)))
    ...
    (let ((new-rest (make-instance 'pose :size (length rest)))
      ...
      (dotimes (i (length rest))
        (let ((this (gethash i forward))
          ...
          (setf (parent-joint rest i) parent)
          (setf (parent-joint bind i) parent)))
...
(defmethod reorder ((mesh mesh) map)
  (let ((data (vertex-data mesh)))
    (loop for i from (+ 3 3 2) below (length data) by (+ 3 3 2 4 4)
          do (setf (aref data (+ i 0)) (gethash (truncate (aref data (+ i 0))) map))
             (setf (aref data (+ i 1)) (gethash (truncate (aref data (+ i 1))) map))
             (setf (aref data (+ i 2)) (gethash (truncate (aref data (+ i 2))) map))
             (setf (aref data (+ i 3)) (gethash (truncate (aref data (+ i 3))) map)))
    mesh))
