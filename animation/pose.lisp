  ...
  (let ((length (length (joints pose)))
    ...
    (loop while (< i length)
          ...
          do (when (< i parent) (return))
             (let ((global (tmat (aref joints i) (svref result i))))
               (when (<= 0 parent)
                 (n*m (aref result parent) global)))
             (incf i))
    (loop while (< i length)
          do (tmat (global-transform pose i) (svref result i))
             (incf i))
    result))

(defmethod dquat-palette ((pose pose) result)
  (let ((length (length (joints pose)))
        (joints (joints pose))
        (parents (parents pose)))
    (setf result (%adjust-array result length #'dquat))
    (loop for i from 0 below length
          for res = (svref result i)
          do (tdquat (svref joints i) res)
             (loop for parent = (aref parents i) then (aref parents parent)
                   while (<= 0 parent)
                   do (let ((temp (dquat)))
                        (declare (dynamic-extent temp))
